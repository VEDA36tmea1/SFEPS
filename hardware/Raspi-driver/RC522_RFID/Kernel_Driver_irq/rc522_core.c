...
int rc522_read_uid_blocking(struct rc522_dev *dev, u32 *out_uid)
{
	...
	while (1) {
		...
		/* 먼저 폴링 방식으로 한 번 시도 (IRQ가 없거나 타이밍이 애매한 경우 대비) */
		if (rc522_read_uid_no_block(dev, out_uid) == 0) {
			atomic_set(&dev->irq_event, 0);
			return 0;
		}

		/* IRQ 기반 모드: 이벤트가 들어올 때까지 대기 (최대 500ms) */
		ret = wait_event_interruptible_timeout(
			dev->waitq,
			atomic_read(&dev->irq_event),
			msecs_to_jiffies(500));
		if (ret < 0)
			return ret; /* 신호 등으로 깨어남 */

		/* timeout 또는 irq_event=1: 다음 루프로 넘어가서 다시 rc522_read_uid_no_block 시도 */
		atomic_set(&dev->irq_event, 0);
	}
}
